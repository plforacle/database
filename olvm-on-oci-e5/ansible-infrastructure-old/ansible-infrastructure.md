...
6. Run the Ansible playbook to create the OCI infrastructure and OLVM instances.
    ...
    **Expected runtime:** 20-35 minutes. If the playbook runs for more than 45 minutes or shows no new task output for more than 10 minutes, stop and contact the instructor or workshop owner before changing the environment manually.
    ...
    > **Stop immediately on a provisioning failure.**
    >
    > Do not continue to later tasks if the output contains `UNREACHABLE!`, `FAILED!`, or a refused SSH connection for `olvm`, `olkvm01`, or `olkvm02`. The infrastructure playbook can enter its cleanup sequence after a failed host configuration. Press **Ctrl+C** as soon as the failure appears, then answer `a` if Ansible asks whether to abort. Do not try to repair the host manually while that playbook process is still running.
    >
    > A common transient failure is `olkvm02 ... port 22: Connection refused` during **Wait for systems to become reachable using ssh**. This means the instance was created but its SSH service was not ready when Ansible attempted configuration. Treat this as a failed deployment, not as a warning that can be ignored. Do not proceed unless the play reaches `PLAY [Print instances]`.

    > **Critical: Preserve the lab environment after a successful deployment.**
    >
    > When the playbook completes successfully, you may see a prompt to remove resources.
    >
    > - Do **not** press Enter
    > - Do **not** type `y`
    > - Press **Ctrl+C**
    ...
    >
    > This preserves the deployed OLVM infrastructure for the remaining labs. This prompt is not a recovery mechanism for an earlier provisioning failure. If a failure occurred first, inspect the OCI Console before rerunning the lab because the playbook might already have started deleting resources.
    ...
    **Recovery after a failed run:**
    ...
    1. In the OCI Console, check whether `olvm`, `olkvm01`, and `olkvm02` still exist and are **Running**. If any are missing or terminating, treat the environment as incomplete and clean up any remaining workshop resources before retrying from Task 2 with a new bootstrap instance.
    2. If all three instances exist and only `olkvm02` failed to answer on port 22, wait 5 minutes and verify that its instance state is **Running**. Do not rerun `create_instance.yml` against that partially configured environment.
    3. Capture the failed task and error output, then stop. The upstream playbook must be corrected to retry SSH readiness for every created host before this lab can safely be rerun.
